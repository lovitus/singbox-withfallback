...
var _ adapter.URLTestGroup = (*Fallback)(nil)
...
type Fallback struct {
	...
	successThreshold             uint16
	group                        *FallbackGroup
	interruptExternalConnections bool
}

func NewFallback(ctx context.Context, _ adapter.Router, logger log.ContextLogger, tag string, options option.FallbackOutboundOptions) (adapter.Outbound, error) {
	outbound := &Fallback{
...
func (s *Fallback) Start() error {
	...
	group, err := NewFallbackGroup(s.ctx, s.outbound, s.logger, outbounds, s.link, s.interval, s.idleTimeout, s.failureThreshold, s.successThreshold, s.interruptExternalConnections)
	if err != nil {
		return err
	}
	s.group = group
...
func (s *Fallback) DialContext(ctx context.Context, network string, destination M.Socksaddr) (net.Conn, error) {
	...
	if err != nil {
		...
		s.group.ReportFailure(selected)
		s.group.history.DeleteURLTestHistory(RealTag(selected))
		return nil, err
	}
...
type fallbackOutboundState struct {
	available bool
	failures  uint16
	successes uint16
}
...
func NewFallbackGroup(ctx context.Context, outboundManager adapter.OutboundManager, logger log.Logger, outbounds []adapter.Outbound, link string, interval time.Duration, idleTimeout time.Duration, failureThreshold uint16, successThreshold uint16, interruptExternalConnections bool) (*FallbackGroup, error) {
	if interval == 0 {
	...
	outboundStatus := make(map[string]fallbackOutboundState)
	for _, detour := range outbounds {
		realTag := RealTag(detour)
		if _, loaded := outboundStatus[realTag]; loaded {
			continue
		}
		outboundStatus[realTag] = fallbackOutboundState{
			available: true,
		}
	}
...
func (g *FallbackGroup) Select(network string) (adapter.Outbound, bool) {
	for _, detour := range g.outbounds {
		...
		if g.isOutboundAvailable(RealTag(detour)) {
			return detour, true
		}
	}
	for _, detour := range g.outbounds {
	...
	for _, detour := range g.outbounds {
		...
		b.Go(realTag, func() (any, error) {
			...
			if err != nil {
				...
				g.markOutboundUnavailable(realTag)
			} else {
				...
				g.markOutboundAvailable(realTag)
				resultAccess.Lock()
				result[tag] = t
				resultAccess.Unlock()
			}
...
func (g *FallbackGroup) ReportFailure(detour adapter.Outbound) {
	realTag := RealTag(detour)
	g.markOutboundUnavailable(realTag)
	g.performUpdateCheck()
}

func (g *FallbackGroup) isOutboundAvailable(realTag string) bool {
	g.statusAccess.Lock()
	defer g.statusAccess.Unlock()
	status, loaded := g.outboundStatus[realTag]
	if !loaded {
		g.outboundStatus[realTag] = fallbackOutboundState{available: true}
		return true
	}
	return status.available
}

func (g *FallbackGroup) markOutboundUnavailable(realTag string) {
	g.statusAccess.Lock()
	defer g.statusAccess.Unlock()
	status, loaded := g.outboundStatus[realTag]
	if !loaded {
		status = fallbackOutboundState{available: true}
	}
	status.successes = 0
	if status.available {
		status.failures++
		if status.failures >= g.failureThreshold {
			status.available = false
			status.failures = 0
		}
	}
	g.outboundStatus[realTag] = status
}

func (g *FallbackGroup) markOutboundAvailable(realTag string) {
	g.statusAccess.Lock()
	defer g.statusAccess.Unlock()
	status, loaded := g.outboundStatus[realTag]
	if !loaded {
	...
	if !status.available {
		status.successes++
		if status.successes >= g.successThreshold {
